...
@interface RNPaypalSdk () <PayPalPaymentDelegate, RCTBridgeModule>
...
RCT_REMAP_METHOD(paymentRequest,
    ...
    if (![requiredParamsSet isEqualToSet:paramsSet]) {
        NSError *error = [NSError errorWithDomain:@"com.RNPaypalSdk" code:-10 userInfo:@{@"Error reason": @"Invalid Input"}];
...
- (void)payPalPaymentDidCancel:(nonnull PayPalPaymentViewController *)paymentViewController {
    [paymentViewController.presentingViewController dismissViewControllerAnimated:YES completion:^{
        if (self.reject) {
            NSError *error = [NSError errorWithDomain:@"com.RNPaypalSdk" code:-40 userInfo:@{@"Error reason": @"Invalid Input"}];
